    ...
    return (
        <>
            <div id="header" className='relative w-full'>
                <Breadcrumb heading={t('modals.compare.title')} subHeading={t('modals.compare.title')} />
            </div>
                        ...
                        <div>
                            ...
                            <div className="compare-table flex">
                                <div className="left lg:w-[240px] w-[170px] flex-shrink-0 border border-line border-r-0 rounded-l-2xl">
                                    <div className="item text-button flex items-center h-[60px] px-8 w-full border-b border-line">{t('compare.rating')}</div>
                                    <div className="item text-button flex items-center h-[60px] px-8 w-full border-b border-line">{t('compare.price')}</div>
                                    <div className="item text-button flex items-center h-[60px] px-8 w-full border-b border-line">{t('compare.type')}</div>
                                    <div className="item text-button flex items-center h-[60px] px-8 w-full border-b border-line">{t('compare.brand')}</div>
                                    <div className="item text-button flex items-center h-[60px] px-8 w-full border-b border-line">{t('compare.size')}</div>
                                    <div className="item text-button flex items-center h-[60px] px-8 w-full border-b border-line">{t('compare.colors')}</div>
                                    <div className="item text-button flex items-center h-[60px] px-8 w-full border-b border-line">{t('compare.material')}</div>
                                    <div className="item text-button flex items-center h-[60px] px-8 w-full border-b border-line">{t('compare.addToCart')}</div>
                                </div>
                                <table className="right border-collapse w-full border-t border-r border-line">
                                    ...
                                    <tr className={`flex w-full items-center`}>
                                        {compareState.compareArray.map((item, index) => (
                                            <td className="w-full border border-line h-[60px] border-t-0 border-r-0" key={index}>
                                                <div className='h-full flex items-center justify-center'>
                                                    <div className='button-main py-1.5 px-5' onClick={() => handleAddToCart(item)}>{t('compare.addToCart')}</div>
                                                </div>
                                            </td>
                                        ))}
                                    </tr>
