    ...
    return (
        <>
            <div id="header" className='relative w-full'>
                <Breadcrumb heading={t('wishlist.heading')} subHeading={t('wishlist.heading')} />
            </div>
                    ...
                    <div className="list-product-block relative">
                        <div className="filter-heading flex items-center justify-between gap-5 flex-wrap">
                            ...
                            <div className="right flex items-center gap-3">
                                <div className="select-block filter-type relative">
                                    ...
                                    >
                                        <option value="" disabled>{t('wishlist.type')}</option>
                                ...
                                <div className="select-block relative">
                                    <select
                                        ...
                                        defaultValue=""
                                    >
                                        <option value="" disabled>{t('wishlist.sorting')}</option>
                                        <option value="soldQuantityHighToLow">{t('wishlist.bestSelling')}</option>
                                        <option value="discountHighToLow">{t('wishlist.bestDiscount')}</option>
                                        <option value="priceHighToLow">{t('wishlist.priceHighToLow')}</option>
                                        <option value="priceLowToHigh">{t('wishlist.priceLowToHigh')}</option>
                                    </select>
                                    <Icon.CaretDown size={12} className='absolute top-1/2 -translate-y-1/2 md:right-4 right-2' />
                                </div>
                            </div>
                        ...
                        <div className="list-filtered flex items-center gap-3 mt-4">
                            <div className="total-product">
                                {totalProducts}
                                <span className='text-secondary pl-1'>{t('wishlist.productsFound')}</span>
                            </div>
                            {
                                (selectedType) && (
                                    <>
                                        ...
                                        >
                                            ...
                                            <span className='text-button-uppercase text-red'>{t('wishlist.clearAll')}</span>
                                        </div>
                                    </>
                                )
                            }
                        ...
                        <div className={`list-product hide-product-sold grid lg:grid-cols-${layoutCol} sm:grid-cols-3 grid-cols-2 sm:gap-[30px] gap-[20px] mt-7`}>
                            {currentProducts.map((item) => (
                                item.id === 'no-data' ? (
                                    <div key={item.id} className="no-data-product">{t('wishlist.noProductsMatch')}</div>
                                ) : (
